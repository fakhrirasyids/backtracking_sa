...
int main() {
    ...
    int inputBoard[3][3] = { {2, 8, 3}, {1, 6, 4}, {7, 0, 5} };
    int goalBoard[3][3] = { {1, 2, 3}, {8, 0, 4}, {7, 6, 5} };

    for (int i = 0; i < n; i++) for (int j = 0; j < n; j++) start.board[i][j] = inputBoard[i][j];
    for (int i = 0; i < n; i++) for (int j = 0; j < n; j++) goal.board[i][j] = goalBoard[i][j];
